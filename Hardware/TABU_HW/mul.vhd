...
architecture rtl of matrix_mul is
    ...
    signal e2_reg : signed(21 downto 0);

    signal e1_r_for_g1 : signed(23 downto 0);
    signal f1_r_for_g1 : signed(23 downto 0); 
    signal sum_g1_temp : signed(23 downto 0);
    signal g1_reg : signed(19 downto 0);

    signal e2_r_for_g2 : signed(23 downto 0);
    signal f2_r_for_g2 : signed(23 downto 0); 
...
begin
    ...
    e1_r_for_g1 <= resize(e1_reg, 24);
    f1_r_for_g1 <= resize(f1 & "000", 24); 
    sum_g1_temp <= e1_r_for_g1 + f1_r_for_g1;

    e2_r_for_g2 <= resize(e2_reg, 24);
    f2_r_for_g2 <= resize(f2 & "0", 24); 
    sum_g2_temp <= e2_r_for_g2 + f2_r_for_g2;

    p_h1g1_full <= resize(h1 * g1_reg, 40);
    p_h2g2_full <= resize(h2 * g2_reg, 37);
    k1_sum_temp <= resize(p_h1g1_full, 41) + resize(p_h2g2_full & "000", 41); 

    p_a1b1_full <= resize(a1 * b1, 32);
    p_a2b2_full <= resize(a2 * b2, 29);
    c1_sum_temp <= resize(p_a1b1_full, 34) + resize(p_a2b2_full & "0", 34); 
    
    o_sum_temp <= resize(k1_reg, 26) + resize(c1_reg & "0", 26);
    ...
    begin
        if rst = '1' then
            ...
            e1_reg <= sum_e1_temp(39 downto 33) & sum_e1_temp(32 downto 17);
            e2_reg <= sum_e2_temp(39 downto 33) & sum_e2_temp(32 downto 18);
            
            g1_reg <= sum_g1_temp(23 downto 18) & sum_g1_temp(17 downto 4);
            g2_reg <= sum_g2_temp(23 downto 18) & sum_g2_temp(17 downto 5);

            k1_reg <= k1_sum_temp(40 downto 33) & k1_sum_temp(32 downto 17);
            
            c1_reg <= c1_sum_temp(33 downto 31) & c1_sum_temp(30 downto 16);
        end if;
    end process;
